...
-(void)loadSubViews {
    ...
    bgImageView.image = [UIImage imageNamed:@"WechatIMG100"];
...
//    [self addSubview:titleView];
    ...
    for (int i = 0; i<2; i++) {
        for (int j = 0; j<images.count; j++) {
            ...
            item.frame = CGRectMake(marginX +(Row+itemW)*(j%3), SCREEN_HIGHT*1.2, itemW, itemH);
    ...
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, Height-69, Width, 49)];
//    _bottomView.backgroundColor = [UIColor redColor];
    [_backGroundView addSubview:_bottomView];
    
    _exitImgvi = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"打叉-(1)"]];
    _exitImgvi.frame = CGRectMake(Width/2-29/2, 10, 40, 40);
