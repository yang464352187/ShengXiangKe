...
@interface MyBussinessBuyCell1 ()<PopView1Delegate>

@end

@implementation MyBussinessBuyCell1{
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _orderNum = [UILabel createLabelWithFrame:VIEWFRAME(150, 60, 100, 50)                                                 andText:@"运单号:"
                                     andTextColor:[UIColor blackColor]
                                       andBgColor:[UIColor clearColor]
                                          andFont:SYSTEMFONT(12)
                                 andTextAlignment:NSTextAlignmentLeft];
...
{
    MyBussinessBuyModel *_model = model;
    
    _title.text = _model.purchase[@"name"];
    _content.text = _model.purchase[@"description"];
    CGFloat height = [UILabel getHeightByWidth:(SCREEN_WIDTH - 150 - 15 -10 -25) title:_model.purchase[@"description"] font:SYSTEMFONT(11)];
    if (height > 26.5) {
        height = 26.5;
    }
    [self reSetSize:height];
    
    _priceTitle.text = [NSString stringWithFormat:@"市场价:¥%.2f",[_model.purchase[@"marketPrice"] floatValue] /100];
    //    _marketPrice.text = [NSString stringWithFormat:@"¥:%.2f",[_model.advancePrice floatValue] /100];
    _price.text = [NSString stringWithFormat:@"¥:%.2f",[_model.purchase[@"advancePrice"] floatValue] /100];
    ...
    [_headImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",APP_BASEIMG,_model.purchase[@"imgList"][0]]] placeholderImage:[UIImage imageNamed:@"占位-0"]];
...
-(void)btnAction:(UIButton *)sender
{
    ...
    [_popView changeTitle2:@"是否确认收货?" andIdex:_rentid type:0];
    [_popView show];
