...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _time = [UILabel createLabelWithFrame:VIEWFRAME(150, 60, 100, 50)                                                 andText:@"租期:"
                                 andTextColor:[UIColor blackColor]
                                   andBgColor:[UIColor clearColor]
                                      andFont:SYSTEMFONT(13)
                             andTextAlignment:NSTextAlignmentLeft];
...
-(void)setModel:(id)model
{
    ...
    _content.text = [NSString stringWithFormat:@"%@ %@",array[[_model.rent[@"condition"] integerValue]],_model.rent[@"keyword"]];
    _time1.text = _model.tenancy[@"name"];
//    NSLog(@"----------%@",_model.tenancy[@"name"]);
    _price.text = [NSString stringWithFormat:@"%.2f元/天",[_model.rent[@"rentPrice"] floatValue]/100];
    _marketPrice.text = [NSString stringWithFormat:@"市场价:¥%.2f",[_model.rent[@"marketPrice"] floatValue] /100];
    
    
    [_headImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",APP_BASEIMG,_model.rent[@"imgList"][0]]]];
}
